<template>
  <div>
    <van-nav-bar left-text="返回" left-arrow title="检测结果查询" @click-left="onClickLeft" />
    <van-image width="100%" height="150" :src="indexpng" fit="fill" />
    <van-notice-bar wrapable left-icon="volume-o" text="1.请输入并核实检测人员姓名和证件号码查询检测结果。2.核酸检测结果分批发布，请您耐心等待并持续关注官方发布消息。" />
    <van-form @submit="onSubmit">
      <van-field v-model="tname" name="姓名" label="姓名" placeholder="姓名" :rules="[{ required: true, message: '请填写姓名' }]" />
      <van-field v-model="idcardnum" name="身份证号码" label="身份证号码" placeholder="身份证号码" :rules="[{ required: true, message: '请填写身份证号码' }]" />
