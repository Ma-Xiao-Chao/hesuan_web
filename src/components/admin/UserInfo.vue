  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        ...
        <el-button type="primary" icon="el-icon-search" @click="getData(currentPage,pageSize)">搜索</el-button>
      </div>
      <el-table
        :data="tableData"
        border
...
export default {
  ...
  methods: {
    ...
    getData(page, size) {
      if (this.query.idcardnum == null || this.query.idcardnum == "") {
        // alert("输入框为空,全查");
        // alert("查询所有数据");
        //alert(this.query.idcardnum)
        this.$axios
          .get("/findAllUser?pageNum="+page+"&pageSize="+size)
          .then(res => {
            if (res.data.code == 1) {
         ...
         alert(this.query.idcardnum)
        this.$axios
          .get("/findUserByIdCard?idcardnum=" + this.query.idcardnum)
          .then(res => {
            if (res.data.code == 1) {
